...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
    ...
    if (isset($_POST['action']) && $_POST['action'] == 'add_game') {
        ...
        if (empty($name)) {
            ...
            $stmt = $conn->prepare("INSERT INTO games (name, provider, type, image_url, status) VALUES (?, ?, ?, ?, ?)");
            $stmt->bind_param("sssss", $name, $provider, $type, $image_url, $status);
    ...
    if (isset($_POST['action']) && $_POST['action'] == 'edit_game') {
        ...
        if (empty($name)) {
            ...
            $stmt = $conn->prepare("UPDATE games SET name = ?, provider = ?, type = ?, image_url = ?, status = ? WHERE id = ?");
            $stmt->bind_param("sssssi", $name, $provider, $type, $image_url, $status, $game_id);
    ...
    $provider_filter = isset($_POST['provider_filter']) ? $_POST['provider_filter'] : '';
    header("Location: manage_games.php" . ($provider_filter ? "?provider=$provider_filter" : ""));
    exit;
}

// Definir o provider atual
$current_provider = isset($_GET['provider']) && in_array($_GET['provider'], ['PG', 'Pragmatic']) ? $_GET['provider'] : 'PG';
...
// Buscar todos os jogos do provider atual
...
$games_query = "SELECT * FROM games {$where_clause} ORDER BY name ASC";
...
// Contar jogos por provider
$pg_count_query = "SELECT COUNT(*) as count FROM games WHERE provider = 'PG'";
$pragmatic_count_query = "SELECT COUNT(*) as count FROM games WHERE provider = 'Pragmatic'";

$pg_count_result = $conn->query($pg_count_query);
$pragmatic_count_result = $conn->query($pragmatic_count_query);

$pg_count = $pg_count_result->fetch_assoc()['count'];
$pragmatic_count = $pragmatic_count_result->fetch_assoc()['count'];

// Definir título da página
$pageTitle = 'Gerenciar Jogos ' . $current_provider;
...
<head>
    ...
    <style>
        ...
        * {
        ...
        .provider-tab.pragmatic {
            border-left: 4px solid var(--pragmatic-color);
        }
        
        .provider-tab.pg.active {
            background: var(--pg-color);
            color: white;
        }
        
        .provider-tab.pragmatic.active {
            background: var(--pragmatic-color);
            color: white;
        }
...
<body>
            ...
            <div class="content">
                ...
                <div class="d-sm-flex align-items-center justify-content-between mb-4">
                    <h1 class="h3 mb-0 text-gray-800">
                        <?php if ($current_provider == 'PG'): ?>
                            <i class="fas fa-dice me-2" style="color: var(--pg-color);"></i> Gerenciar Jogos PG
                        <?php else: ?>
                            <i class="fas fa-gamepad me-2" style="color: var(--pragmatic-color);"></i> Gerenciar Jogos Pragmatic
                        <?php endif; ?>
                    </h1>
                    
                    <button type="button" class="btn btn-primary" data-bs-toggle="modal" data-bs-target="#addGameModal">
                        <i class="fas fa-plus-circle me-1"></i> Adicionar Novo Jogo <?php echo $current_provider; ?>
                    </button>
                </div>
                ...
                <div class="provider-tabs">
                    <a href="manage_games.php?provider=PG" class="provider-tab pg <?php echo $current_provider == 'PG' ? 'active' : ''; ?>">
                        <i class="fas fa-dice mb-2"></i>
                        <span>Jogos PG</span>
                        <span class="badge bg-<?php echo $current_provider == 'PG' ? 'light text-dark' : 'primary'; ?>"><?php echo $pg_count; ?></span>
                    </a>
                    
                    <a href="manage_games.php?provider=Pragmatic" class="provider-tab pragmatic <?php echo $current_provider == 'Pragmatic' ? 'active' : ''; ?>">
                        <i class="fas fa-gamepad mb-2"></i>
                        <span>Jogos Pragmatic</span>
                        <span class="badge bg-<?php echo $current_provider == 'Pragmatic' ? 'light text-dark' : 'primary'; ?>"><?php echo $pragmatic_count; ?></span>
                    </a>
                </div>
                
                <!-- Barra de pesquisa -->
                <div class="search-container">
                    <form action="manage_games.php" method="GET" class="search-form">
                        <input type="hidden" name="provider" value="<?php echo $current_provider; ?>">
                        <input type="text" name="search" class="form-control search-input" placeholder="Pesquisar jogos <?php echo $current_provider; ?>..." value="<?php echo htmlspecialchars($search_query); ?>">
                ...
                <div class="games-grid">
                    ...
                    <div class="game-card" data-status="<?php echo $game['status']; ?>">
                        ...
                        <div class="game-type-badge <?php echo $game['provider'] == 'PG' ? 'pg' : 'pragmatic'; ?>">
                            <i class="<?php echo $game['provider'] == 'PG' ? 'fas fa-dice' : 'fas fa-gamepad'; ?>"></i>
                            <?php echo $game['provider']; ?>
                        </div>
                        ...
                        <div class="game-body">
                            <div class="game-meta">
                                ...
                                <div class="d-flex align-items-center">
                                    <span class="text-secondary"><i class="fas fa-tag me-2"></i> Tipo:</span>
                                    <span class="ms-auto"><?php echo htmlspecialchars($game['type']); ?></span>
                                </div>
                                <?php endif; ?>
                            </div>
                        </div>
                        ...
                        <div class="game-footer">
                            <button type="button" class="btn btn-info btn-action" data-bs-toggle="modal" data-bs-target="#editGameModal"
                                ...
                                data-provider="<?php echo $game['provider']; ?>"
                                data-type="<?php echo htmlspecialchars($game['type'] ?? ''); ?>"
                                data-image-url="<?php echo htmlspecialchars($game['image_url'] ?? ''); ?>"
                                data-status="<?php echo $game['status']; ?>"
                                title="Editar Jogo">
                                <i class="fas fa-edit"></i>
                            </button>
                            
                            <form method="post" action="manage_games.php" class="d-inline" onsubmit="return confirm('Tem certeza que deseja alterar o status deste jogo?')">
                                <input type="hidden" name="action" value="toggle_status">
                                <input type="hidden" name="game_id" value="<?php echo $game['id']; ?>">
                                <input type="hidden" name="current_status" value="<?php echo $game['status']; ?>">
                                <input type="hidden" name="provider_filter" value="<?php echo $current_provider; ?>">
                            ...
                            <form method="post" action="manage_games.php" class="d-inline" onsubmit="return confirm('Tem certeza que deseja excluir este jogo? Esta ação não pode ser desfeita.')">
                ...
                <div class="text-center py-5">
                    <i class="fas <?php echo $current_provider == 'PG' ? 'fa-dice' : 'fa-gamepad'; ?> fa-3x mb-3 text-secondary"></i>
                    <h4>Nenhum jogo encontrado</h4>
                    <p class="text-muted">
                        <?php if (!empty($search_query)): ?>
                            Nenhum jogo encontrado para a pesquisa "<?php echo htmlspecialchars($search_query); ?>".
                        <?php else: ?>
                            Clique em "Adicionar Novo Jogo <?php echo $current_provider; ?>" para começar.
                        <?php endif; ?>
                    </p>
                </div>
                <?php endif; ?>
        ...
        <div class="modal-dialog modal-dialog-centered">
            <div class="modal-content">
                <div class="modal-header">
                    <h5 class="modal-title">Adicionar Novo Jogo <?php echo $current_provider; ?></h5>
                    <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
                </div>
                <div class="modal-body">
                    <form method="post" action="manage_games.php" id="addGameForm">
                        ...
                        <div class="mb-3">
                            <label for="type" class="form-label">Tipo do Jogo (opcional)</label>
                            <input type="text" class="form-control" id="type" name="type">
                            <div class="form-text">Categoria ou tipo específico do jogo</div>
                        </div>
                        
                        <div class="mb-3">
                            <label for="image_url" class="form-label">URL da Imagem (opcional)</label>
                            <input type="url" class="form-control" id="image_url" name="image_url">
                            <div class="form-text">Link para a imagem do jogo</div>
                        </div>
    ...
    <div class="modal fade" id="editGameModal" tabindex="-1" aria-hidden="true">
        <div class="modal-dialog modal-dialog-centered">
            <div class="modal-content">
                ...
                <div class="modal-body">
                    <form method="post" action="manage_games.php" id="editGameForm">
                        ...
                        <div class="mb-3">
                            ...
                            <div class="d-flex gap-4">
                                <div class="form-check">
                                    <input class="form-check-input" type="radio" name="provider" id="provider_pg" value="PG">
                                    <label class="form-check-label" for="provider_pg">
                                        <i class="fas fa-dice me-1" style="color: #9c27b0;"></i> PG
                                    </label>
                                </div>
                                <div class="form-check">
                        ...
                        <div class="mb-3">
                            <label for="edit_type" class="form-label">Tipo do Jogo (opcional)</label>
                            <input type="text" class="form-control" id="edit_type" name="type">
                            <div class="form-text">Categoria ou tipo específico do jogo</div>
                        </div>
                        
                        <div class="mb-3">
                            <label for="edit_image_url" class="form-label">URL da Imagem (opcional)</label>
    ...
    document.addEventListener('DOMContentLoaded', function() {
        ...
        // Filtros de status (all/active/inactive)
        ...
        filterItems.forEach(item => {
            item.addEventListener('click', function(e) {
                ...
                if (this.classList.contains('filter-status')) {
                    const status = this.getAttribute('data-status');
                    
                    // Filter game cards by status
                ...
                } else if (this.classList.contains('filter-all')) {
                    // Show all cards
                    gameCards.forEach(card => {
                        card.style.display = 'flex';
                    });
        ...
        if (editGameModal) {
            editGameModal.addEventListener('show.bs.modal', function(event) {
                ...
                if (provider === 'PG') {
                    document.getElementById('provider_pg').checked = true;
                } else {
                    document.getElementById('provider_pragmatic').checked = true;
                }
